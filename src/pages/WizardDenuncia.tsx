...
const WizardDenuncia: React.FC = () => {
  ...
  // 🚨 VERIFICAÇÃO DE PERMISSÕES COM DEBUG
  useEffect(() => {
    console.log('🔍 [WIZARD] Debug Permissões:');
    console.log('🔍 [WIZARD] User:', user);
    console.log('🔍 [WIZARD] hasPermission(admin):', hasPermission('admin'));
    console.log('🔍 [WIZARD] User perfil:', user?.perfil);
    console.log('🔍 [WIZARD] User ativo:', user?.ativo);
    
    // 🚨 TEMPORARIAMENTE REMOVENDO A VERIFICAÇÃO PARA TESTE
    ...
    toast({
      title: "🔍 DEBUG MODE",
      description: `Usuário: ${user.username} | Perfil: ${user.perfil} | Acesso liberado para teste`,
      variant: "default",
    });
  ...
  const loadInitialData = async () => {
    try {
      ...
      console.error('❌ [WIZARD] Erro ao carregar dados:', error);
      toast({
        title: "❌ Erro",
                    ...
                    <Card key={index} className="border-2 border-green-200 bg-green-50">
                      <CardHeader className="bg-green-100 border-b border-green-200">
                        <CardTitle className="text-lg text-green-800 flex items-center">
                          <Target className="h-5 w-5 mr-2" />
                          ALVO #{index + 1} - {formData.formData?.data_denuncia ? `${formData.data_denuncia.slice(2,4)}${String(index + 1).padStart(3, '0')}-ANIM${String(index + 1).padStart(2, '0')}` : `ANIM${String(index + 1).padStart(2, '0')}`}
                        </CardTitle>
                      </CardHeader>
