...
const AdministracaoSimples = () => {
  ...
  const fetchCategorias = async () => {
    try {
      debugLogger.log('info', 'SIMPLES: Carregando categorias...');
      ...
      if (error) {
        debugLogger.log('error', 'SIMPLES: Erro ao carregar', error);
        throw error;
      }

      debugLogger.log('success', `SIMPLES: ${data?.length || 0} categorias carregadas`, data?.slice(0, 2));
      setCategoriasFinanceiras(data || []);

    } catch (error: any) {
      debugLogger.log('error', 'SIMPLES: Erro geral', error);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      debugLogger.log('error', 'Erro ao salvar categoria', error);
      toast({
        title: "Erro",
  ...
  const toggleActive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      debugLogger.log('error', 'Erro ao alterar status', error);
      toast({
        title: "Erro",
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      ...
      debugLogger.log('error', 'Erro ao eliminar categoria', error);
      toast({
        title: "Erro",
  ...
  debugLogger.log('debug', `SIMPLES: Renderizando com ${filteredData.length} categorias filtradas`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-yellow-50 to-green-50">
      <UserHeader 
        title="Gestão de Categorias Financeiras" 
        description="Versão simplificada para debug"
        showBackButton
      ...
      {/* Debug Logger */}
      <DebugLoggerComponent title="Categorias Simples - Debug" />
    </div>
  );
};

export default AdministracaoSimples;
