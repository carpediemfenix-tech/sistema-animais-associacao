-- ESTRUTURA COMPLETA PARA RELATÓRIOS E ANALYTICS
-- Sistema avançado de relatórios, métricas e dashboard
...
SELECT 
    ...
    AVG(CASE 
        ...
        THEN EXTRACT(days FROM a.data_devolucao::date - a.data_devolucao_prevista::date)
        ELSE NULL 
...
SELECT 
    ...
    CASE 
        ...
        THEN EXTRACT(days FROM CURRENT_DATE - e.data_aquisicao::date) / 365.0
        ELSE 0 
    END as idade_anos,
    CASE 
        WHEN e.valor_aquisicao > 0 AND e.data_aquisicao IS NOT NULL
        THEN (e.valor_aquisicao + COALESCE(SUM(m.custo), 0)) / GREATEST(EXTRACT(days FROM CURRENT_DATE - e.data_aquisicao::date) / 365.0, 0.1)
        ELSE 0 
...
SELECT 
    ...
    AVG(CASE 
        ...
        THEN EXTRACT(days FROM CURRENT_DATE - data_aquisicao::date) / 365.0
        ELSE NULL 
...
SELECT 
    ...
    AVG(CASE 
        ...
        THEN EXTRACT(days FROM data_resolucao - data_criacao)
        ELSE NULL 
...
SELECT 
    ...
    AVG(CASE 
        ...
        THEN EXTRACT(days FROM data_devolucao::date - data_atribuicao::date)
        ELSE NULL 
...
-- 7. Criar função para gerar relatório completo de equipamento
CREATE OR REPLACE FUNCTION gerar_relatorio_equipamento(equipamento_uuid UUID)
RETURNS JSON AS $$
DECLARE
    resultado JSON;
BEGIN
    SELECT json_build_object(
        'equipamento', json_build_object(
            'id', e.id,
        ...
        'estatisticas', json_build_object(
            'total_atribuicoes', COALESCE(stats.total_atribuicoes, 0),
            'atribuicoes_ativas', COALESCE(stats.atribuicoes_ativas, 0),
            'total_manutencoes', COALESCE(stats.total_manutencoes, 0),
            'custo_total_manutencoes', COALESCE(stats.custo_total_manutencoes, 0),
            'tempo_medio_uso_dias', COALESCE(stats.tempo_medio_uso_dias, 0),
            'idade_anos', COALESCE(stats.idade_anos, 0)
        ),
        'historico_atribuicoes', COALESCE(atrib.historico, '[]'::json),
        'historico_manutencoes', COALESCE(manut.historico, '[]'::json),
        'alertas_ativos', COALESCE(alert.alertas, '[]'::json)
    ) INTO resultado
    FROM equipamentos_2025_12_13_01_00 e
    LEFT JOIN tipos_equipamentos_2025_12_13_01_00 te ON e.tipo_equipamento_id = te.id
    LEFT JOIN categorias_equipamentos_2025_12_13_01_00 ce ON te.categoria_id = ce.id
    ...
    ) stats ON e.id = stats.equipamento_id
    LEFT JOIN (
        SELECT 
            equipamento_id,
            COUNT(*) as total_manutencoes,
    ...
    LEFT JOIN (
        SELECT 
            ...
            ) ORDER BY a.data_atribuicao DESC) as historico
        FROM atribuicoes_equipamentos_2025_12_13_01_00 a
        LEFT JOIN voluntarios v ON a.voluntario_id = v.id
        GROUP BY a.equipamento_id
    ) atrib ON e.id = atrib.equipamento_id
    LEFT JOIN (
        SELECT 
            equipamento_id,
            json_agg(json_build_object(
                ...
                'custo', custo,
                'status', status,
                'fornecedor', fornecedor_servico
            ) ORDER BY data_manutencao DESC) as historico
        FROM manutencoes_equipamentos_2025_12_13_01_00
        GROUP BY equipamento_id
    ) manut ON e.id = manut.equipamento_id
    LEFT JOIN (
        SELECT 
            equipamento_id,
            json_agg(json_build_object(
            ...
            ) ORDER BY data_criacao DESC) as alertas
        FROM alertas_equipamentos_2025_12_16_07_00
        WHERE status = 'ativo'
        GROUP BY equipamento_id
    ) alert ON e.id = alert.equipamento_id
    WHERE e.id = equipamento_uuid;
    
    RETURN resultado;
END;
$$ LANGUAGE plpgsql;

-- 8. Criar função para KPIs do dashboard
...
-- 9. Verificar se as views foram criadas corretamente
SELECT 
