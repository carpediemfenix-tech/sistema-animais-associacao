...
AND column_name IN ('quantidade_devolvida', 'quantidade_restante', 'estado');

-- Verificar se a função existe
SELECT routine_name, routine_type 
FROM information_schema.routines 
WHERE routine_name = 'processar_devolucao_parcial_item';

-- Recriar função de devolução parcial melhorada
...
BEGIN
    ...
    INSERT INTO movimentos_stock_2026_01_06 (
        ...
        v_item.quantidade_atual,
        v_item.quantidade_atual + p_quantidade_devolver,
    ...
    RETURN json_build_object(
        ...
        'stock_atualizado', v_item.quantidade_atual + p_quantidade_devolver
    );
...
-- Testar a função com dados de exemplo (se existirem)
DO $$
DECLARE
    v_test_result JSON;
BEGIN
    -- Verificar se existe alguma atribuição ativa para teste
    IF EXISTS (SELECT 1 FROM atribuicoes_itens_2026_01_07_00_52 WHERE estado IN ('ATIVO', 'PARCIALMENTE_DEVOLVIDO') LIMIT 1) THEN
        RAISE NOTICE 'Função processar_devolucao_parcial_item criada e pronta para uso';
    ELSE
        RAISE NOTICE 'Função processar_devolucao_parcial_item criada. Nenhuma atribuição ativa encontrada para teste';
    END IF;
END;
$$;