...
-- Políticas RLS para localizacoes_animal
CREATE POLICY IF NOT EXISTS "Public can view animal locations" ON localizacoes_animal
FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can insert animal locations" ON localizacoes_animal
FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Authenticated users can update animal locations" ON localizacoes_animal
FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Authenticated users can delete animal locations" ON localizacoes_animal
