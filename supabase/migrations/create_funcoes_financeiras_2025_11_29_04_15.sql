...
-- Função para calcular custo total de um animal
...
BEGIN
    ...
    custos_responsabilidades AS (
        SELECT COALESCE(SUM(
            CASE 
                WHEN ra.data_fim IS NOT NULL THEN 
                    EXTRACT(EPOCH FROM (ra.data_fim - ra.data_inicio)) / (30 * 24 * 60 * 60) * sr.subsidio_mensal
                ELSE 
                    EXTRACT(EPOCH FROM (CURRENT_DATE - ra.data_inicio)) / (30 * 24 * 60 * 60) * sr.subsidio_mensal
            END
        ), 0) as total_responsabilidades
        ...
        WHERE ra.animal_id = animal_uuid
    )
    SELECT 
...
-- Função para calcular custos por categoria
...
BEGIN
    ...
    FROM intervencoes
    UNION ALL
    SELECT 'Localizações'::TEXT, COALESCE(SUM(
        CASE 
            WHEN la.data_fim IS NOT NULL THEN 
                (la.data_fim - la.data_inicio) * cl.custo_diario
            ELSE 
    ...
    LEFT JOIN custos_localizacoes cl ON la.tipo_localizacao = cl.tipo_localizacao
    UNION ALL
    SELECT 'Responsabilidades'::TEXT, COALESCE(SUM(
        CASE 
            WHEN ra.data_fim IS NOT NULL THEN 
                EXTRACT(EPOCH FROM (ra.data_fim - ra.data_inicio)) / (30 * 24 * 60 * 60) * sr.subsidio_mensal
            ELSE 
...
RETURNS TABLE (
    ...
    custo_total_associacao DECIMAL(10,2)
) AS $$
BEGIN
    RETURN QUERY
    WITH custos_totais AS (
        SELECT SUM(custo_total) as total_geral
        FROM (
            SELECT calcular_custo_total_animal(id) as custo_total
            FROM animais
        ) custos
    )
    SELECT 
        ...
        COALESCE(ct.total_geral, 0)
    FROM custos_totais ct;
END;
$$ LANGUAGE plpgsql;

-- Função para obter top 10 animais com maiores custos
...
BEGIN
    ...
    SELECT 
        ...
        a.nome,
        a.especie,
        (calcular_custo_total_animal(a.id)).custo_total
    FROM animais a
    ORDER BY (calcular_custo_total_animal(a.id)).custo_total DESC
    LIMIT 10;
END;
$$ LANGUAGE plpgsql;

-- Função para obter custos por mês (últimos 12 meses)
...
BEGIN
    ...
    WITH meses AS (
        SELECT 
            TO_CHAR(date_trunc('month', CURRENT_DATE - INTERVAL '11 months') + INTERVAL '1 month' * generate_series(0, 11), 'YYYY-MM') as mes
    ),
    custos_intervencoes_mes AS (
        ...
        FROM intervencoes
        WHERE data_intervencao >= CURRENT_DATE - INTERVAL '12 months'
...
SELECT 'Teste função custos por mês:' as teste;
SELECT * FROM get_custos_por_mes() LIMIT 3;