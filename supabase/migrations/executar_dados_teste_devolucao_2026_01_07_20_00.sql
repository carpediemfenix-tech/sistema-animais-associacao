...
SELECT 
    ...
    i.nome as item,
    i.quantidade_atual as stock_atual,
...
SELECT 
    'ATRIBUIÇÃO CRIADA' as status,
    a.entidade_nome as item,
    a.quantidade_atribuida as stock_atual,
    a.estado as resultado
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM itens_aprovisionamento_2026_01_06 WHERE nome = 'TESTE DEVOLUÇÃO - Caneca Azul') THEN
        ...
        RAISE NOTICE '4. Clicar no botão "Devolver"';
        RAISE NOTICE '5. Testar devolver 4 unidades (de 10)';
        RAISE NOTICE '6. Resultado esperado: 6 restantes, stock +4';
    ELSE
        RAISE NOTICE '❌ ERRO: Dados de teste não foram criados';
    END IF;
END;
$$;