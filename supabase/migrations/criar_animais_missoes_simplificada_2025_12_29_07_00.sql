-- =====================================================
-- CRIAR TABELA DE ANIMAIS DE MISSÕES - SIMPLIFICADA
-- Data: 2025-12-29 17:00
-- Objetivo: Criar tabela funcional para animais de missões
-- =====================================================

-- FASE 1: Criar nova tabela de animais de missões
...
-- FASE 2: Inserir dados de exemplo
DO $$
DECLARE
    missao_id UUID;
    animal_id UUID;
...
BEGIN
    ...
    -- Inserir vinculações de exemplo para cada missão
    FOR missao_id IN 
        SELECT id FROM missoes_2025_12_29_07_00 LIMIT 3
    LOOP
        -- Vincular 1-2 animais por missão
        FOR animal_id IN 
        ...
        LOOP
            ...
            VALUES 
            (missao_id, animal_id, 'participante', CURRENT_DATE, 'ativa', 'Vinculação de exemplo')
    ...
    RAISE NOTICE '  ✅ % vinculações de exemplo criadas', inserted_count;
END $$;

-- FASE 3: Remover tabelas antigas
DO $$
DECLARE
    tabela_nome TEXT;
    tamanho_tabela TEXT;
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '🗑️ REMOVENDO TABELAS ANTIGAS';
    RAISE NOTICE '===========================';
...
BEGIN
    ...
    RAISE NOTICE '✅ Total de registos: %', total_registos;
    ...
    FOR rec IN 
        ...
        AND table_name = 'missoes_animais_2025_12_29_07_00'
