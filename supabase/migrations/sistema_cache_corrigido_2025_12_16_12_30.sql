-- ========================================
-- SISTEMA DE CACHE E OTIMIZAÇÃO (CORRIGIDO)
-- ========================================
...
-- View otimizada para estatísticas de animais (corrigida)
...
SELECT 
    ...
    -- Correção: cast para text antes de usar regex
    AVG(CASE WHEN idade_estimada::text ~ '^[0-9]+$' THEN idade_estimada::INTEGER ELSE NULL END) as idade_media
FROM public.animais;
...
SELECT 
    ...
    -- Correção: verificar se valor_aquisicao não é null antes de cast
    SUM(CASE 
        WHEN valor_aquisicao IS NOT NULL AND valor_aquisicao::text ~ '^[0-9]+\.?[0-9]*$' 
        THEN valor_aquisicao::NUMERIC 
        ELSE 0 
    END) as valor_total_inventario,
...
SELECT 'Sistema de cache implementado com sucesso!' as status;