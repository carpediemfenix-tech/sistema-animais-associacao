...
-- 2. INSERIR ITENS DE EXEMPLO (baseados nos seus exemplos)
...
-- Exemplos com stock baixo para demonstrar alertas
((SELECT id FROM tipos_aprovisionamento_2026_01_06 WHERE nome = 'Ração Cão Adulto'), 
 'Ração Seca Cão Sénior 15kg', 'Ração seca para cães seniores', '15kg', 
 2, 5, 38.00, 'Armazém A - Prateleira 1'),

((SELECT id FROM tipos_aprovisionamento_2026_01_06 WHERE nome = 'Colete Refletor'), 
 'Blusão Valentão ao Resgate M', 'Blusão oficial da associação tamanho M', 'M', 
 1, 3, 25.00, 'Armazém B - Armário Fardamento');
...
-- 4. MOSTRAR RESUMO DOS ITENS CRIADOS
SELECT 
    c.nome as categoria,
    t.nome as tipo,
    i.nome as item,
...
-- 5. MOSTRAR ALERTAS DE STOCK BAIXO
SELECT 
