-- Criar tabelas para gestão de campos com opções
...
-- Inserir dados padrão para especialidades
...
-- Políticas RLS - Escrita apenas para administradores (verificar na tabela users)
CREATE POLICY "Escrita espécies para administradores" ON public.especies_opcoes
    FOR ALL USING (
        EXISTS (
...
CREATE POLICY "Escrita sexos para administradores" ON public.sexos_opcoes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = auth.uid() 
...
CREATE POLICY "Escrita especialidades para administradores" ON public.especialidades_opcoes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = auth.uid() 
...
CREATE POLICY "Escrita estados para administradores" ON public.estados_opcoes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = auth.uid() 
...
CREATE POLICY "Escrita tipos intervenções para administradores" ON public.tipos_intervencoes_opcoes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = auth.uid() 
