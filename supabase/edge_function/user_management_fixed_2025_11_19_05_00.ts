...
serve(async (req) => {
  ...
  try {
    console.log('👥 [USER_MGMT] Iniciando gestão de utilizadores...')
    
    // Verificar variáveis de ambiente
    const supabaseUrl = Deno.env.get('SUPABASE_URL')
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')
    
    console.log('🔧 [USER_MGMT] Variáveis de ambiente:', {
      supabaseUrl: supabaseUrl ? 'OK' : 'MISSING',
      supabaseServiceKey: supabaseServiceKey ? 'OK' : 'MISSING'
    })

    if (!supabaseUrl || !supabaseServiceKey) {
      console.error('❌ [USER_MGMT] Variáveis de ambiente em falta')
      return new Response(
        JSON.stringify({ 
          success: false, 
    ...
    const checkAdminPermission = async (userId: string) => {
      ...
      console.log('👤 [USER_MGMT] Utilizador encontrado:', {
        found: !!user,
    ...
    if (method === 'GET') {
      ...
      console.log('📋 [USER_MGMT] Listando utilizadores para:', currentUserId)
      
      if (!currentUserId || !(await checkAdminPermission(currentUserId))) {
        console.log('❌ [USER_MGMT] Acesso negado')
        return new Response(
      ...
      console.log('✅ [USER_MGMT] Utilizadores encontrados:', users?.length || 0)
      return new Response(
    ...
    if (method === 'POST') {
      ...
      console.log('➕ [USER_MGMT] Criando utilizador:', {
        ...
        nome_completo: requestBody.nome_completo,
        perfil_acesso: requestBody.perfil_acesso,
        current_user_id: requestBody.current_user_id
      })
      ...
      if (!current_user_id || !(await checkAdminPermission(current_user_id))) {
        console.log('❌ [USER_MGMT] Acesso negado para criação')
        return new Response(
          JSON.stringify({ success: false, error: 'Acesso negado. Apenas administradores.' }),
          { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      ...
      if (!username || !email || !password || !nome_completo || !perfil_acesso) {
        console.log('❌ [USER_MGMT] Campos obrigatórios em falta')
        return new Response(
      ...
      if (password.length < 6) {
        console.log('❌ [USER_MGMT] Password muito curta')
        return new Response(
      ...
      console.log('🔍 [USER_MGMT] Verificando se username existe:', username)
      ...
      if (existingUser) {
        console.log('❌ [USER_MGMT] Username já existe')
        return new Response(
      ...
      console.log('🔍 [USER_MGMT] Verificando se email existe:', email)
      ...
      if (existingEmail) {
        console.log('❌ [USER_MGMT] Email já existe')
        return new Response(
          JSON.stringify({ success: false, error: 'Email já existe' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      // Encriptar password
      console.log('🔐 [USER_MGMT] Encriptando password...')
      let passwordHash
      try {
        passwordHash = await bcrypt.hash(password, 10)
        console.log('✅ [USER_MGMT] Password encriptada com sucesso')
      } catch (bcryptError) {
        console.error('❌ [USER_MGMT] Erro ao encriptar password:', bcryptError)
        return new Response(
      ...
      // Criar utilizador
      console.log('💾 [USER_MGMT] Inserindo utilizador na base de dados...')
      ...
      try {
        ...
        console.log('📝 [USER_MGMT] Log de atividade criado')
      } catch (logError) {
        console.error('❌ [USER_MGMT] Erro ao criar log:', logError)
      }
    ...
    if (method === 'PUT') {
      const requestBody = await req.json()
      console.log('✏️ [USER_MGMT] Atualizando utilizador:', requestBody.id)
    ...
    if (method === 'PATCH') {
      const requestBody = await req.json()
      console.log('🔑 [USER_MGMT] Resetando password para:', requestBody.user_id)

      const { user_id, new_password, current_user_id }: ResetPasswordRequest = requestBody
